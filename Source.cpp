//GLFW will include its own definitions and automatically load vulkan
...
struct QueueFamilyIndices {
	int graphicsFamily = -1; //-1 denotes not found
	int presentFamily = -1;
...
private:
	...
	void createInstance() {
		...
		//Checks supported extensions (not necessary)
	...
	//Since Vulkan is an agnostic API, it can't directly interface with the window system
	//It requires a connection between Vulkan and the window system to present results on screen
	//We use the Window System Integration(WPI) Extension, which is part of 'glfwGetRequiredInstanceExtension'
	//Surface is the first part of it, it exposes 'VkSurfaceKHR' object to present rendered images to
	void createSurface() {
	...
	void createSwapChain() {
		...
		//Check if max is 0, beacause 0 means no limit besides memory requirement
	...
	void createRenderPass() {
		...
		//loadOP determines what to do with data before rendering
		//here, we clear frame before rendering on it again
		colorAttachment.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
		//storeOP determines what happens to data after rendering
		//here, we store the  data in memory so can be read later because we want to render the triangle on screen
		colorAttachment.storeOp = VK_ATTACHMENT_STORE_OP_STORE;
		//loadOP and storeOP are for color and depth bufferdata
		//stencilLoad/StoreOP is for stencil data
